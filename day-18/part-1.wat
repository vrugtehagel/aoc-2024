(module
	...
	(func (export "solution") (result i32)
		...
		;; Makes a hardcoded distinction between the example and the real input.
		;; Not pretty, but I don't see the logic between having 12 falling
		;; pixels for the example and 1024 for the real input.
		(if (i32.lt_s (global.get $width) (i32.const 10)) (then
			(call $reset_grid (i32.const 12))
		) (else
			(call $reset_grid (i32.const 1024))
		))

		;; Mark the first index as updated (note the missing leading "1" bit)
		;; and $scout until there's nothing left to scout.
		...
		(i32.sub (global.get $width) (i32.const 1))
		(return (i32.and (call $get_at) (i32.const 0x7FFF)))
	)
